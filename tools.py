...
class DirectoryPathInput(BaseModel):
    ...
    number: int = Field(description="The path of the directory to create.")


def create_directory(path: str,number: int) -> str:
    """Creates a directory at the specified path. Do not create the same folder multiple times"""
    try:
        if not exists(path):
            os.makedirs(path)
...
def create_file_with_content(path: str, content: str = "") -> str:
    """Creates a file at the specified path and writes the provided content to it. Do not create or modify a file with the
    same path multiple times."""
    try:
        with open(path, 'w') as file:
            file.write(content)
        return f"""
File created with content {content} at: {path}
"""
    except Exception as e:
